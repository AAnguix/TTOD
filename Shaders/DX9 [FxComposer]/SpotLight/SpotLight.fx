...
{  
	...
	float l_AngleAttenuation=25.0;
	float l_FallOffAttenuation=45.0;
	float l_CosAngle=cos(l_AngleAttenuation*0.5*3.1416/180.0);
	float l_CosFallOff=cos(l_FallOffAttenuation*0.5*3.1416/180.0);
	float l_DotLight=dot(lightDirection, -Lamp0Direction);
	...
	float3 l_DiffuseLight=l_DiffuseContrib*Lamp_0_color.xyz*l_Texture.xyz*g_OmniLightIntensity*l_Attenuation*l_SpotAttenuation;
	
	float3 l_SpecularLight=l_SpecularContrib * Lamp_0_color.xyz * g_OmniLightIntensity * l_Attenuation*l_SpotAttenuation; 
