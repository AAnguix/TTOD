...
//Necesitas: Diffuso, normales, profundidades.

static float m_RawDataValues[64]=((float[64])m_RawData);
static float m_HDAOActive=m_RawDataValues[0];
static bool m_HDAOShowNormals=m_RawDataValues[1]==1.0;
static bool m_HDAOShowAO=m_RawDataValues[2]==1.0;
static bool m_HDAOUseNormals=m_RawDataValues[3]==1.0;
static float2 m_RTSize=float2(m_RawDataValues[4], m_RawDataValues[5]);
static float m_AcceptAngle=m_RawDataValues[6];
static float m_HDAOIntensity=m_RawDataValues[7];
//static float g_fQTimesZNearKK=m_RawDataValues[8];
//static float g_fQKK=m_RawDataValues[9];
static float m_NormalScale=m_RawDataValues[8];
static float m_HDAORejectRadius=m_RawDataValues[9];
static float m_HDAOAcceptRadius=m_RawDataValues[10];
...
float NormalRejectionTest(int2 i2ScreenCoord, float2 RTSize)
{
	...
	for( int iNormal=0; iNormal<NUM_NORMAL_LOADS; iNormal++ )
	{
		...
		i2OffsetScreenCoord = ( i2OffsetScreenCoord > ( RTSize - float2( 1.0f, 1.0f ) ) ) ? ( RTSize -
		float2( 1.0f, 1.0f ) ) : ( i2OffsetScreenCoord );
		i2MirrorOffsetScreenCoord = ( i2MirrorOffsetScreenCoord > ( RTSize - float2( 1.0f, 1.0f ) ) ) ?
		( RTSize - float2( 1.0f, 1.0f ) ) : ( i2MirrorOffsetScreenCoord );
		i2OffsetScreenCoord = ( i2OffsetScreenCoord < 0 ) ? ( 0 ) : ( i2OffsetScreenCoord );
		i2MirrorOffsetScreenCoord = ( i2MirrorOffsetScreenCoord < 0 ) ? ( 0 ) : (
		i2MirrorOffsetScreenCoord );
...
float4 PS(PS_INPUT IN) : SV_Target
{
	...
	int iGather;
	float fDot = 1.0f;
	float2 f2KernelScale = float2( m_RTSize.x / 1024.0f, m_RTSize.y / 768.0f );
	float3 f3CameraPos;
	...
	if( fDot > 0.5f )
	{
		...
		{
			f2MirrorScreenCoord = ( ( f2KernelScale * m_HDAORingPattern[iGather] ) + float2( 1.0f,
			1.0f ) ) * float2( -1.0f, -1.0f );
			f2TexCoord = float2( ( f2ScreenCoord + ( f2KernelScale * m_HDAORingPattern[iGather] ) )
			* f2InvRTSize );
			f2MirrorTexCoord = float2( ( f2ScreenCoord + ( f2MirrorScreenCoord ) ) * f2InvRTSize );
			// Sample
			f2TexCoord_10_1 = float2( ( f2ScreenCoord_10_1 + ( ( f2KernelScale *
			m_HDAORingPattern[iGather] ) + float2( 1.0f, 1.0f ) ) ) * f2InvRTSize );
			f2MirrorTexCoord_10_1 = float2( ( f2ScreenCoord_10_1 + (
			f2MirrorScreenCoord + float2( 1.0f, 1.0f ) ) ) * f2InvRTSize );
			f4SampledZ[0] = GatherZSamples( T2Texture, S2Sampler,
			f2TexCoord_10_1, l_Q, l_QTimesZNear);
			f4SampledZ[1] = GatherZSamples( T2Texture, S2Sampler,
			f2MirrorTexCoord_10_1, l_Q, l_QTimesZNear);
			// Detect valleys
	...
	if( m_HDAOUseNormals )
	{
		fOcclusion = ( ( f4Occlusion.x + f4Occlusion.y + f4Occlusion.z + f4Occlusion.w ) / ( 3.0f *
		m_RingWeightsTotal[iNumRings - 1] ) );
	}
	else
	{
		fOcclusion = ( ( f4Occlusion.x + f4Occlusion.y + f4Occlusion.z + f4Occlusion.w ) / ( 2.0f *
		m_RingWeightsTotal[iNumRings - 1] ) );
	}
