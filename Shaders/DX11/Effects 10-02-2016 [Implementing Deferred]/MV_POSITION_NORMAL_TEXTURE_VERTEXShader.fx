...
float4 PS( PS_INPUT IN) : SV_Target
{
	...
	{ 
		if(m_LightEnabledArray[i]==1.0f) //Enabled
		{
			...
			else if(m_LightTypeArray[i]==1.0f) //DIRECTIONAL
			{
				...
				float l_DiffuseContrib=max(0.0, dot(Nn, -m_LightDirection[i]));
	...
	return float4( clamp(float3(l_AmbientLight),0,1) + clamp(float3(l_SpecularLight),0,1) + clamp(float3(l_DiffuseLight),0,1), l_Texture.a );	
}